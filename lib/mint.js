...
async function requestAirdropAndConfirm() {
    const mint = await createMint(connection, payer, payer.publicKey, freezeAuthority.publicKey, 9 // We are using 9 to match the CLI decimal default exactly
    );
    ...
    await mintTo(connection, payer, mint, tokenAccount.address, payer, 100000000000 // because decimals for the mint are set to 9 
    );
    const mintInf = await getMint(connection, mint);
    console.log(mintInf.supply);
    await burn(connection, payer, tokenAccount.address, mint, payer, 100000000000 // because decimals for the mint are set to 9 
    );
    const mintIn = await getMint(connection, mint);
